...
fn sessions_with_failed_tool_filters() {
    ...
    write_trace(
        dir.path(),
        "s1",
        &[ev("s1", "tool_result", "read_file", "error")],
    );
    // s2: read_file succeeded (no error).
    write_trace(
        dir.path(),
        "s2",
        &[ev("s2", "tool_result", "read_file", "info")],
    );
    // s3: a different tool errored (must not match read_file).
    write_trace(
        dir.path(),
        "s3",
        &[ev("s3", "tool_result", "grep", "error")],
    );
    ...
    assert!(
        ...
        "s1 hit a read_file error"
    );
    assert!(!sessions.contains(&"s2".to_string()), "s2 did not error");
    assert!(
        !sessions.contains(&"s3".to_string()),
        "s3 errored on a different tool"
    );
}
...
fn empty_traces_dir_returns_empty() {
    ...
    let ta = TraceAnalytics::open(dir.path()).unwrap();
    assert!(ta
        .sessions_with_failed_tool("read_file")
        .unwrap()
        .is_empty());
}
