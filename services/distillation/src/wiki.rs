...
fn parse_compilation_response(text: &str) -> Result<CompilationResponse, DistillationError> {
    ...
    serde_json::from_str(json_text)
        .map_err(|e| DistillationError::from(format!("Failed to parse compilation response: {e}")))
}

async fn build_index_content(
    ward_id: &str,
    wiki_store: &dyn WikiStore,
) -> Result<String, DistillationError> {
