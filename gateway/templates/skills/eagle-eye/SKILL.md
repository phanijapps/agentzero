---
name: "eagle-eye"
description: "Visual intelligence skill — use when asked to analyze images, screenshots, diagrams, charts, or any task requiring sight. Extracts structured insights from visual content using the multimodal_analyze tool. Works even when the current agent runs on a text-only model."
allowed-tools: multimodal_analyze
metadata:
  author: "agentzero"
  version: "1.0.1"
  tags: "vision,multimodal,image-analysis"
  trigger_keywords: "image,screenshot,diagram,chart,visual,picture,photo,analyze image,describe image,what do you see,look at this"
  domain_hints: "vision,multimodal,visual-analysis"
---

# Eagle Eye — Visual Intelligence

You have the ability to see. Use the `multimodal_analyze` tool to process any visual content — images, screenshots, diagrams, charts, or photos.
...
4. **File paths** can be absolute paths, `file://` URIs, or URLs. The tool resolves them automatically (URLs are fetched and inlined; only images are supported).
...
- Documents and PDFs are not supported — use shell tools to extract their text instead.
